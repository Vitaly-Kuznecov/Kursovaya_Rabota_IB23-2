...
dark_theme = {'bg': 'gray20', 'fg': 'white'}

def toggle_theme():
    global is_dark_theme
    is_dark_theme = not is_dark_theme
    theme = dark_theme if is_dark_theme else light_theme
    update_colors(root, theme)

def update_colors(widget, theme):
    ...
    for child in widget.winfo_children():
        update_colors(child, theme)
def show_help():
    messagebox.showinfo("Справка",
                        'Программа для шифрования по алгоритму "Кузнечик" ГОСТ Р 34.12-2015\n'
                        "Разработчик: Кузнецов Виталий Вадимович\n"
                        "Почта: vkuznecov055@gmail.com")

def save_key():
    key = key_entry.get().strip()
    if not key:
        messagebox.showerror("Ошибка", "Сначала сгенерируйте или введите ключ.")
        return
    file_path = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[("Text files", "*.txt")])
    if file_path:
        with open(file_path, 'w') as file:
            file.write(key)
        messagebox.showinfo("Успех", "Ключ успешно сохранен.")
...
def xor_func(input1AsString, input2AsString, in_code=16):
    ...
    resultAsHex = hex(result).upper()[2:]
...
galua_fields = [
                ...
                248, 51, 102, 204, 91, 182, 175, 157, 249, 49, 98, 196, 75, 150, 239, 29,
                58, 116, 232, 19, 38, 76, 152, 243, 37, 74, 148, 235, 21, 42, 84, 168, 147, 229,
                9, 18, 36, 72, 144, 227, 5, 10, 20, 40, 80, 160, 131, 197, 73, 146, 231, 13, 26,
                52, 104, 208, 99, 198, 79, 158, 255, 61, 122, 244, 43, 86, 172, 155, 245, 41, 82,
                164, 139, 213, 105, 210, 103, 206, 95, 190, 191, 189, 185, 177, 161, 129, 193, 65,
                130, 199, 77, 154, 247, 45, 90, 180, 171, 149, 233, 17, 34, 68, 136, 211, 101, 202,
                87, 174, 159, 253, 57, 114, 228, 11, 22, 44, 88, 176, 163, 133, 201, 81, 162, 135,
                205, 89, 178, 167, 141, 217, 113, 226, 7, 14, 28, 56, 112, 224, 3, 6, 12, 24, 48, 96,
                192, 67, 134, 207, 93, 186, 183, 173, 153, 241, 33, 66, 132, 203, 85, 170, 151, 237,
                25, 50, 100, 200, 83, 166, 143, 221, 121, 242, 39, 78, 156, 251, 53, 106, 212, 107,
                214, 111, 222, 127, 254, 63, 126, 252, 59, 118, 236, 27, 54, 108, 216, 115, 230, 15,
                30, 60, 120, 240, 35, 70, 140, 219, 117, 234, 23, 46, 92, 184, 179, 165, 137, 209, 97,
                194, 71, 142, 223, 125, 250, 55, 110, 220, 123, 246, 47, 94, 188, 187, 181, 169, 145, 225, 1]
...
nonlinear_coef = [252, 238, 221, 17, 207, 110, 49, 22, 251, 196, 250, 218, 35, 197, 4,
...
def decrypt(text, K):
    ...
    text_dec = ''.join(textDecrypt)
    try:
        text_dec = binascii.unhexlify(text_dec).decode('utf8')
    except:
        pass
    return text_dec
...
def encrypt_text():
    ...
    if len(key) != 64 or not all(c in '0123456789ABCDEFabcdef' for c in key):
        messagebox.showerror("Ошибка", "Ключ должен содержать ровно 64 шестнадцатеричных символа.")
        return
    text = encrypt_input_text.get("1.0", tk.END).strip()
    if not text:
    ...
    encrypt_output_text.insert("1.0", encrypted)
...
def decrypt_text():
    ...
    if len(key) != 64 or not all(c in '0123456789ABCDEFabcdef' for c in key):
        messagebox.showerror("Ошибка", "Ключ должен содержать ровно 64 шестнадцатеричных символа.")
        return
    text = decrypt_input_text.get("1.0", tk.END).strip()
    if not text:
        messagebox.showerror("Ошибка", "Введите текст для расшифровки.")
        return
    if len(text) % 2 != 0 or not all(c in '0123456789ABCDEFabcdef' for c in text):
        messagebox.showerror("Ошибка", "Текст для расшифровки должен быть валидной шестнадцатеричной строкой.")
        return
    K = getKeys(key.upper())
    decrypted = decrypt(text.upper(), K)
    decrypt_output_text.delete("1.0", tk.END)
    decrypt_output_text.insert("1.0", decrypted)
root = tk.Tk()
root.title('Шифр "Кузнечик" ГОСТ Р 34.12-2015')
# root.iconbitmap('cryptography.ico')
top_frame = tk.Frame(root)
top_frame.pack(side=tk.TOP, fill=tk.X)
help_btn = tk.Button(top_frame, text="Справка", command=show_help)
help_btn.pack(side=tk.LEFT, padx=5, pady=5)
theme_btn = tk.Button(top_frame, text="Переключить тему", command=toggle_theme)
theme_btn.pack(side=tk.LEFT, padx=5, pady=5)
key_frame = tk.Frame(root)
key_frame.pack(pady=10)
tk.Label(key_frame, text="Ключ:").pack(side=tk.LEFT)
key_entry = tk.Entry(key_frame, width=70)
key_entry.pack(side=tk.LEFT, padx=5)
generate_key_btn = tk.Button(key_frame, text="Сгенерировать ключ", command=generate_key)
generate_key_btn.pack(side=tk.LEFT)
save_key_btn = tk.Button(key_frame, text="Сохранить ключ", command=save_key)
save_key_btn.pack(side=tk.LEFT, padx=5)
main_frame = tk.Frame(root)
main_frame.pack(pady=10)
encrypt_frame = tk.Frame(main_frame)
encrypt_frame.pack(side=tk.LEFT, padx=20)
tk.Label(encrypt_frame, text="Текст для шифрования:").pack()
encrypt_input_text = tk.Text(encrypt_frame, height=10, width=50)
encrypt_input_text.pack()
encrypt_btn = tk.Button(encrypt_frame, text="Зашифровать", command=encrypt_text)
encrypt_btn.pack(pady=5)
tk.Label(encrypt_frame, text="Зашифрованный текст:").pack()
encrypt_output_text = tk.Text(encrypt_frame, height=10, width=50)
encrypt_output_text.pack()
decrypt_frame = tk.Frame(main_frame)
decrypt_frame.pack(side=tk.LEFT, padx=20)
tk.Label(decrypt_frame, text="Текст для расшифровки:").pack()
decrypt_input_text = tk.Text(decrypt_frame, height=10, width=50)
decrypt_input_text.pack()
decrypt_btn = tk.Button(decrypt_frame, text="Расшифровать", command=decrypt_text)
decrypt_btn.pack(pady=5)
tk.Label(decrypt_frame, text="Расшифрованный текст:").pack()
decrypt_output_text = tk.Text(decrypt_frame, height=10, width=50)
decrypt_output_text.pack()
update_colors(root, light_theme)
root.mainloop()